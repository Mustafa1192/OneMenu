...
// import products from "./data/Frankies";
...
const FreshJuice = () => {
  ...
  const loadProducts = async () => {
    try {
      // const response = await axios.get('http://localhost:5000/api/products/selected-menus');
      const response = await axios.get('http://localhost:5000/api/products/frankies_Rolls');
      setVisibleProducts(response.data.slice(0, page * productsPerPage)); // Show products based on page count
    } catch (error) {
  ...
  useEffect(() => {
    loadProducts(); // Call API to fetch products when the page changes
  }, [page]);
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ([entry]) => {
        if (entry.isIntersecting) {
          setPage((prevPage) => prevPage + 1); // Load more products when bottom is reached
        }
      },
      { threshold: 1.0 }
    );
              ...
              <div className="flex items-center">
                {Array.from({ length: 5 }, (_, index) => (
                  <svg
                    key={index}
                    xmlns="http://www.w3.org/2000/svg"
                    className={`w-3 h-3 ${index < Math.floor(product.rating) ? "text-yellow-400" : "text-gray-300"}`}
                    fill="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 17.27L18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21z" />
