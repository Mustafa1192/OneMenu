...
// import products from "./data/FreshJuice";
...
const FreshJuice = () => {
  ...
  const loadMoreProducts = () => {
    const nextProducts = visibleProducts.slice(0, page * productsPerPage);
    setVisibleProducts(nextProducts);
  };
  ...
  useEffect(() => {
    // Use Axios to fetch products from the backend
    const fetchProducts = async () => {
      try {
        // const response = await axios.get('http://localhost:5000/api/products/selected-menus');
        const response = await axios.get('http://localhost:5000/api/products/FreshJuice');
        setVisibleProducts(response.data); // Assuming the API returns an array of products
      } catch (error) {
